...
TEST_CASE( "Shapes" ) {
    ...
    SECTION( "Box" ) {    
        TGeoVolume *top = gGeoManager->MakeBox("BOX",Vacuum,10,20,30);
        gGeoManager->SetTopVolume(top);
        gGeoManager->CloseGeometry();

        json j_box = exp.convert(geom);
        //std::ofstream fout("comp_test_ROOT_box.json"); 
        //fout << j_box.dump(4) << std::endl;
        REQUIRE( !j_box.empty() );
    } 

    SECTION( "Cone" ) {
        TGeoVolume *top = gGeoManager->MakeCone("CONE",Vacuum, 40,10,20,35,45);
        gGeoManager->SetTopVolume(top);
        gGeoManager->CloseGeometry();

        json j_cone = exp.convert(geom);
        //std::ofstream fout("comp_test_ROOT_cone.json"); 
        //fout << j_cone.dump(4) << std::endl;
        REQUIRE( !j_cone.empty() );
    } 
    ...
    SECTION( "big root test" ) {
        ...
        TGeoArb8 *arb = dynamic_cast<TGeoArb8*>(bar3->GetShape());
        ...
        json j_root = exp.convert(geom);

        // std::ofstream fout("comp_test_ROOT.json");
        // fout << j.dump(4) << std::endl; 

        REQUIRE( !j_root.empty() );
    }
}